...
from gsstudio.utils.base import Updateable, update_end_if_possible, update_if_possible
from gsstudio.utils.config import C, parse_optimizer, parse_scheduler, parse_structured
from gsstudio.utils.misc import cleanup, find_last_path, get_device, load_module_weights
from gsstudio.utils.saving import SaverMixin
...
class BaseLift3DSystem(BaseSystem):
    ...
    def configure(self) -> None:
        ...
        if (
            ...
            gsstudio.info("Initializing geometry from a given checkpoint ...")
            from gsstudio.utils.config import load_config, parse_structured
            ...
            prev_geometry = gsstudio.find(prev_system_cfg.geometry_type)(
                prev_geometry_cfg
            )
            state_dict, epoch, global_step = load_module_weights(
            ...
            self.geometry = gsstudio.find(self.cfg.geometry_type).create_from(
                prev_geometry,
                self.cfg.geometry,
            ...
            self.geometry = gsstudio.find(self.cfg.geometry_type)(self.cfg.geometry)

        self.material = gsstudio.find(self.cfg.material_type)(self.cfg.material)
        self.background = gsstudio.find(self.cfg.background_type)(self.cfg.background)
        self.renderer = gsstudio.find(self.cfg.renderer_type)(
            self.cfg.renderer,
    ...
    def on_fit_start(self) -> None:
        if self._save_dir is not None:
            gsstudio.info(f"Validation results will be saved to {self._save_dir}")
        else:
            gsstudio.warn(
    ...
    def on_test_end(self) -> None:
        if self._save_dir is not None:
            gsstudio.info(f"Test results saved to {self._save_dir}")

    def on_predict_start(self) -> None:
        self.exporter: Exporter = gsstudio.find(self.cfg.exporter_type)(
            self.cfg.exporter,
            geometry=self.geometry,
            material=self.material,
            background=self.background,
    ...
    def on_predict_end(self) -> None:
        if self._save_dir is not None:
            gsstudio.info(f"Export assets saved to {self._save_dir}")
