...
@threestudio.register("gaussian-splatting-planes")
class GaussianBaseModel(BaseGeometry):
    ...
    def get_features(self):
        color = self.feature_network(self.encoding(self.get_xyz.detach())).unsqueeze(1)
        # features_dc = torch.tanh(self._features_dc + color)
        features_dc = color
    ...
    def get_opacity(self):
        opacity = self.density_network(self.encoding(self.get_xyz.detach())) - 2.5
    ...
    def training_setup(self):
        ...
        l = [
            ...
            },
            {
                ...
                "lr": 0.001,
                "name": "planesdensity_network",
            },
            {
                "params": self.feature_network.parameters(),
                "lr": 0.001,
                "name": "planesfeature_network",
            },
            {
                "params": [self._xyz],
    ...
    def _prune_optimizer(self, mask):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if len(group["params"]) == 1 and (not group["name"].startswith("planes")):
                stored_state = self.optimizer.state.get(group["params"][0], None)
    ...
    def densify_and_split(self, grads, grad_threshold, N=2):
        ...
        new_scaling = self.scaling_inverse_activation(
            self.get_scaling[selected_pts_mask].repeat(N, 1) / math.pow(N, 1 / 3)
        )
    ...
    def prune(self, min_opacity, max_screen_size):
        ...
        # tiny_points_vs = torch.norm(self.get_scaling, dim=1).values < self.cfg.split_thresh / 16
        # prune_mask = torch.logical_or(prune_mask, tiny_points_vs)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        ...
        # self.cfg.densify_grad_threshold = torch.mean(self.xyz_gradient_accum / self.denom) * 1.25
