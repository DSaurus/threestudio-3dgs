...
def join_cameras_as_batch(cameras_list: Sequence[CamerasBase]) -> CamerasBase:
    ...
    for _f in shared_fields:
        _f0 = getattr(c0, _f)
        for _c in cameras_list[1:]:
            if getattr(_c, _f) != _f0:
                raise ValueError(f"Shared field {_f} is not consistent across inputs")

    for _f in fields:
        field_not_none = [(getattr(c, _f) is not None) for c in cameras_list]
        if not any(field_not_none):
            continue
        if not all(field_not_none):
            raise ValueError(f"Attribute {_f} is inconsistently present")

        attrs_list = [getattr(c, _f) for c in cameras_list]

        if isinstance(attrs_list[0], torch.Tensor):
            ...
            _cat_f = torch.cat(attrs_list, dim=0)
            setattr(c0, _f, _cat_f)
        else:
            raise ValueError(f"Field {_f} type is not supported for batching")
        
    setattr(c0, "_N", len(cameras_list))
    return c0
